...
def context_vim_patch_script() -> list[str]:
    return [
        "function! s:PatchContextVim() abort",
        "  if exists('g:nvimview_disable_context_vim_patch')",
        "        \\ && g:nvimview_disable_context_vim_patch",
        "    return",
        "  endif",
        "  if !has('nvim-0.9.0') || !exists(':ContextUpdate')",
        "    return",
        "  endif",
        "lua << NVIMVIEW_CONTEXT",
        "local ok, mod = pcall(require, 'context.highlight')",
        "if not ok or type(mod) ~= 'table' then",
        "  return",
        "end",
        "if mod._nvimview_sync_patch then",
        "  if type(mod.clear_cache) == 'function' then",
        "    pcall(mod.clear_cache)",
        "  end",
        "  return",
        "end",
        "local api = vim.api",
        "local cache = {}",
        "local attached = {}",
        "local old_clear_cache = mod.clear_cache",
        "local function fallback()",
        "  local context = rawget(vim.g, 'context')",
        (
            "  if type(context) == 'table' "
            "and type(context.highlight_normal) == 'string' then"
        ),
        "    return context.highlight_normal",
        "  end",
        "  return 'Normal'",
        "end",
        ...
        "    cache = {}",
        "  end",
        "  if type(old_clear_cache) == 'function' then",
        "    pcall(old_clear_cache, buf)",
        "  end",
        "end",
        "local function usable_hl(name)",
        "  if type(name) ~= 'string' or name == '' then",
        "    return nil",
        "  end",
        "  local ok_hl, hl = pcall(api.nvim_get_hl_by_name, name, true)",
        "  if ok_hl and type(hl) == 'table' and hl[true] ~= 6 then",
        "    return name",
        "  end",
        "  return nil",
        "end",
        "local function compute_hl(buf, row, col)",
        "  if type(vim.inspect_pos) ~= 'function' then",
        "    return fallback()",
        "  end",
        (
            "  local ok_pos, pos = pcall(vim.inspect_pos, buf, row - 1, col, "
            "{ extmarks = false })"
        ),
        "  if not ok_pos or type(pos) ~= 'table' then",
        "    return fallback()",
        "  end",
        "  local semantic_hl = nil",
        "  for _, token in ipairs(pos.semantic_tokens or {}) do",
        "    local opts = token.opts or {}",
        "    semantic_hl = usable_hl(opts.hl_group) or semantic_hl",
        "  end",
        "  if semantic_hl then",
        "    return semantic_hl",
        "  end",
        "  local tree_hl = ''",
        "  for _, token in ipairs(pos.treesitter or {}) do",
        "    local capture = token.capture or ''",
        (
            "    if not capture:match('^_') and capture ~= 'spell' "
            "and capture ~= 'conceal' then"
        ),
        "      tree_hl = token.hl_group or tree_hl",
        "    end",
        "  end",
        "  if tree_hl ~= '' then",
        "    return tree_hl",
        "  end",
        "  local syntax_hl = ''",
        "  for _, token in ipairs(pos.syntax or {}) do",
        "    syntax_hl = token.hl_group or syntax_hl",
        "  end",
        "  if syntax_hl ~= '' then",
        "    return syntax_hl",
        "  end",
        "  return fallback()",
        "end",
        "local function bucket_for(buf)",
        "  if not attached[buf] then",
        "    pcall(api.nvim_buf_attach, buf, false, {",
        "      on_detach = function()",
        "        cache[buf] = nil",
        "        attached[buf] = nil",
        "      end,",
        ...
        "  return cache[buf]",
        "end",
        "mod.nvim_hlgroup = function(winid, row, col)",
        "  local ok_buf, buf = pcall(api.nvim_win_get_buf, winid)",
        "  if not ok_buf then",
        "    return fallback()",
        "  end",
        "  local bucket = bucket_for(buf)",
        "  bucket[row] = bucket[row] or {}",
        "  if bucket[row][col] ~= nil then",
        "    return bucket[row][col]",
        "  end",
        (
            "  local ok_line, lines = pcall(api.nvim_buf_get_lines, "
            "buf, row - 1, row, false)"
        ),
        "  if ok_line and lines and lines[1] and col == #lines[1] then",
        "    bucket[row][col] = fallback()",
        "  else",
        "    bucket[row][col] = compute_hl(buf, row, col)",
        "  end",
        "  return bucket[row][col]",
        "end",
        "mod.clear_cache = clear_cache",
        "mod._nvimview_sync_patch = true",
        "NVIMVIEW_CONTEXT",
        "  silent! ContextUpdate",
        "endfunction",
        "augroup NvimViewContextVim",
        "  autocmd!",
        "  autocmd VimEnter,ColorScheme * call <SID>PatchContextVim()",
        "augroup END",
        "call <SID>PatchContextVim()",
    ]
